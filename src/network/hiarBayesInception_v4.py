...
from keras.layers import Input, merge, Dropout, Dense, Flatten, Activation, Conv2D, concatenate, GlobalAveragePooling2D, Lambda
...
def conv_block(x, nb_filter, nb_row, nb_col, border_mode='same', subsample=(1, 1), bias=False):
    ...
    x = Conv2D(nb_filter, (nb_row, nb_col), strides=subsample, padding=border_mode, use_bias=bias)(x)
...
def hiarBayesInception_v4(height, width, depth, nb_classes, load_weights=True):
    '''
    ...
    if K.image_dim_ordering() == 'th':
        init = Input((depth, height, width))
    else:
        init = Input((height, width, depth))
    ...
    fea_low = Conv2D(256, (3, 3), padding='same', activation='relu', name='conv_low')(fea_low)
    fea_low = GlobalAveragePooling2D()(fea_low)#, name="gap_low"
    fea_low = Dense(256, activation='relu', name='dense_low')(fea_low)
    fea_mid = Conv2D(512, (3, 3), padding='same', activation='relu', name='conv_mid')(fea_mid)
    fea_mid = GlobalAveragePooling2D()(fea_mid)#, name="gap_mid"
    fea_mid = Dense(512, activation='relu', name='dense_mid')(fea_mid)
    fea_hig = Conv2D(1024, (3, 3), padding='same', activation='relu', name='conv_hig')(x)
    fea_hig = GlobalAveragePooling2D()(fea_hig)#, name="gap_hig"
    fea_hig = Dense(1024, activation='relu', name='dense_hig')(fea_hig)
    predictions_low = Dense(nb_classes[0], name="low", activation="sigmoid")(fea_low)#
    predictions_mid = Dense(nb_classes[1], name="middle", activation="sigmoid")(fea_mid)#
    predictions_hig = Dense(nb_classes[2], name="high", activation="sigmoid")(fea_hig)#
    predictions_priori = concatenate([predictions_low, predictions_mid], axis=1)
    predictions_hig_cond = Dense(nb_classes[2], activation="sigmoid", name="high_cond")(predictions_priori)
    predictions_hig_posterior = Lambda(lambda x:x[1] * x[0], name="high_post")([predictions_hig_cond, predictions_hig])
    out = concatenate([predictions_low, predictions_mid, predictions_hig_posterior], axis=1)
    ...
    model = Model(init, out, name='Hiar-Bayes-Inception-v4')
...
if __name__ == "__main__":
    ...
    inception_v4 = hiarBayesInception_v4(299, 299, 3, [10, 10, 10], load_weights=True)
