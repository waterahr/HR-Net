...
class hiarBayesGoogLeNet:
    ...
    def build(width, height, depth, classes, pooling_regions = [1, 3], weights="imagenet"):
        ...
        #fea_mid_at = Flatten()(fea_mid_ot)
        #fea_mid_at = Dense(512, activation='relu')(fea_mid_ot)
        fea_mid_at = GlobalAveragePooling2D()(fea_mid_at)
        predictions_mid_at = Dense(classes[4], name="middle_at", activation="sigmoid")(fea_mid_at)#
        ...
        predictions_hig = Dense(classes[5], name="high_fea", activation="sigmoid")(fea_hig)
        #"""
        """PCM2018"""
        #predictions_hig = Dense(classes[2], activation="sigmoid", name="high")(concatenate([fea_low, fea_mid, fea_hig], axis=1))
        """PCM2018"""
        predictions_priori = concatenate([predictions_low, predictions_mid_ub, predictions_mid_lb, predictions_mid_sh, predictions_mid_at], axis=1)
        """mar"""
        ...
        """mar"""
        predictions_hig_cond = Dense(classes[5], name="high_cond", activation="sigmoid")(predictions_priori)#
        ...
        """
        # create the model
        model = Model(inpt, [predictions_low, predictions_mid_ub, predictions_mid_lb, predictions_mid_sh, predictions_mid_at, predictions_hig_posterior], name='inception')
        if weights == "imagenet":
