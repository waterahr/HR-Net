...
class PolicyGradient:
    def __init__(self,
        ...
        if output_graph:
            tf.summary.FileWriter("../../results/logs/RL_brain_mlp_labels", self.sess.graph)
    ...
    def _build_net(self):
        ...
        #print(tf.expand_dims(prob_logs_low, -1))###Tensor("ExpandDims:0", shape=(?, 61, 1), dtype=float32)
        #print(prob_logs_hig.shape)###(?, 61)
        prob_logs = tf.concat([tf.expand_dims(prob_logs_low, -1), tf.expand_dims(prob_logs_mid, -1), tf.expand_dims(prob_logs_hig, -1)], -1)
    ...
    def learn(self):
        ...
        infor = self.saver.save(self.sess, 
                        '../models/imagenet_models/rl_test15/'+self.reward_type+'/RL_brain_mlp_labels-step'+str(self.step),
                        global_step=1, 
